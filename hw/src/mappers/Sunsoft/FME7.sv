module FME7 (
    ...
    assign bus.prg_oe   = bus.cpu_rw && (bus.cpu_addr[15] || (!wram_select && bus.cpu_addr[15:13] == 3'b011));
    assign bus.prg_we   = !bus.cpu_rw && bus.wram_ce;
    assign bus.wram_ce  = wram_select && wram_enable;
        ...
        if (bus.reset) begin
            ...
            if (bus.sst_we) begin
                case (bus.sst_addr)
                    ...
                    13: {wram_select, mirroring, irq_enabled, irq_counter_enable, irq_carry, irq_pending, wram_enable} <= bus.sst_data_in;
            ...
            if (irq_counter_enable) begin
                ...
                if (irq_enabled && irq_carry) irq_pending <= 1;
            end
            ...
            if (write_en) begin
                if (bus.cpu_addr[14:13] == 2'b00) begin
                    ...
                    case (command)
                        ...
                        8: {wram_enable, wram_select, prg_bank[0]} <= {bus.cpu_data_in[7:6], bus.cpu_data_in[4:0]};
    ...
    assign bus.sst_data_out = (bus.sst_addr == 0) ? {4'b0, command} :
                              ...
                              (bus.sst_addr == 13) ? {wram_select, mirroring, irq_enabled, irq_counter_enable, irq_carry, irq_pending, wram_enable} :
