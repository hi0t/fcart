module ym2149 (
    ...
    localparam logic [11:0] VOL_TABLE[16] = '{
        12'd0,
        12'd14,
        12'd23,
        12'd35,
        ...
        if (reset) begin
            ...
            if (sst_we) begin
                case (sst_addr)
                    ...
                    30: count_a[7:0] <= sst_data_in;
                    31: count_a[11:8] <= sst_data_in[3:0];
                    32: count_b[7:0] <= sst_data_in;
                    33: count_b[11:8] <= sst_data_in[3:0];
                    34: count_c[7:0] <= sst_data_in;
                    35: count_c[11:8] <= sst_data_in[3:0];
                    36: {out_a, out_b, out_c, out_noise} <= sst_data_in[3:0];
                    37: count_noise <= sst_data_in[4:0];
                    38: lfsr_noise[7:0] <= sst_data_in;
                    39: lfsr_noise[15:8] <= sst_data_in;
                    40: lfsr_noise[16] <= sst_data_in[0];
                    41: prescaler <= sst_data_in[4:0];
                    42: selected_reg <= sst_data_in[3:0];
                endcase
            end
        end else begin
            ...
            if (cpu_we) begin
                if (cpu_addr[14:13] == 2'b10) begin  // C000-DFFF
                    ...
                    if (selected_reg[3:1] != 3'b111) regs[selected_reg] <= cpu_data_in;
                end
            end
    ...
    assign amp_a = a_on ? VOL_TABLE[vol_a] : 12'd0;
    assign amp_b = b_on ? VOL_TABLE[vol_b] : 12'd0;
    assign amp_c = c_on ? VOL_TABLE[vol_c] : 12'd0;
    ...
    always_comb begin
        case (sst_addr)
            ...
            30: sst_data_out = count_a[7:0];
            31: sst_data_out = {4'b0, count_a[11:8]};
            32: sst_data_out = count_b[7:0];
            33: sst_data_out = {4'b0, count_b[11:8]};
            34: sst_data_out = count_c[7:0];
            35: sst_data_out = {4'b0, count_c[11:8]};
            36: sst_data_out = {4'b0, out_a, out_b, out_c, out_noise};
            37: sst_data_out = {3'b0, count_noise};
            38: sst_data_out = lfsr_noise[7:0];
            39: sst_data_out = lfsr_noise[15:8];
            40: sst_data_out = {7'b0, lfsr_noise[16]};
            41: sst_data_out = {3'b0, prescaler};
            42: sst_data_out = {4'b0, selected_reg};
