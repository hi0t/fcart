module sdram #(
    ...
    localparam USER_ADDR_BITS = 1 + COLUMN_BITS + ADDR_BITS;
        ...
        case (state)
            ...
            STATE_ACTIVE: begin
                ...
                if (port0.step == ACTIVE_IDLE && port1.step == ACTIVE_IDLE && pending_refresh) begin
                    ...
                    cmd <= CMD_AUTO_REFRESH;
                    state <= STATE_REFRESH;
                end else if (port0.step == ACTIVE_IDLE && ch0.req != ch0.ack && !pending_refresh && !skip_ba) begin
                    port0.step <= 4'(ACTIVE_START);
                    port0.bank <= {1'b0, ch0.address[USER_ADDR_BITS-1-:1]};
                    port0.col <= ch0.address[ADDR_BITS+:COLUMN_BITS];
                    port0.data <= ch0.we ? ch0.data_write : 'x;
                    port0.we <= ch0.we;

                    SDRAM_BA <= {1'b0, ch0.address[USER_ADDR_BITS-1-:1]};
                    SDRAM_ADDR <= ch0.address[ADDR_BITS-1:0];
                    cmd <= CMD_ACTIVATE;
                    skip_ba <= 1;  // Skip bank activation next cycle to satisfy tRRD.
                end else if (port1.step == ACTIVE_IDLE && ch1.req != ch1.ack && !pending_refresh && !skip_ba) begin
                    port1.step <= 4'(ACTIVE_START);
                    port1.bank <= {1'b1, ch1.address[USER_ADDR_BITS-1-:1]};
                    port1.col <= ch1.address[ADDR_BITS+:COLUMN_BITS];
                    port1.data <= ch1.we ? ch1.data_write : 'x;
                    port1.we <= ch1.we;

                    SDRAM_BA <= {1'b1, ch1.address[USER_ADDR_BITS-1-:1]};
                    SDRAM_ADDR <= ch1.address[ADDR_BITS-1:0];
                    cmd <= CMD_ACTIVATE;
                    skip_ba <= 1;
                end else if (port0.step == ACTIVE_CMD) begin  // CAS stage
                    SDRAM_BA <= port0.bank;
                    ...
                    if (port0.we) begin
                        cmd <= CMD_WRITE;
                        data_tx <= port0.data;
                    end else cmd <= CMD_READ;

                    port0.step <= port0.step + 1'd1;
                end else if (port1.step == ACTIVE_CMD) begin
                    SDRAM_BA <= port1.bank;
